 ...
 * #L%
 */
package org.digicart.common.money;
...
import org.digicart.common.money.util.CurrencyAdapter;
import org.digicart.common.util.xml.BigDecimalRoundingAdapter;
import org.digicart.common.web.DigiCartRequestContext;
...
public class Money implements Serializable, Cloneable, Comparable<Money>, Externalizable {
    ...
    protected static String getCurrencyCode(DigiCartCurrency blCurrency) {
        if (blCurrency != null) {
    ...
    public Money(DigiCartCurrency blCurrency) {
        this(0, Currency.getInstance(getCurrencyCode(blCurrency)));
    }

    public Money(BigDecimal amount, DigiCartCurrency blCurrency) {
        this(amount, Currency.getInstance(getCurrencyCode(blCurrency)));
    }

    public Money(BigDecimal amount, DigiCartCurrency blCurrency, int scale) {
    ...
    public static Currency defaultCurrency() {
     ...
     DigiCartRequestContext brc = DigiCartRequestContext.getDigiCartRequestContext();

        if (brc != null && brc.getDigiCartCurrency() != null) {
            assert brc.getDigiCartCurrency().getCurrencyCode()!=null;
            return Currency.getInstance(brc.getDigiCartCurrency().getCurrencyCode());
        }
